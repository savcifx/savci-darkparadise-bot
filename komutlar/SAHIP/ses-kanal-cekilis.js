...
exports.run = async function (client, message, args) {
  ...
  let booster = await db.get(`sunucuayar.booster_rol`)
...
**Doğru Kullanım:** \`${prefix[0]}çekiliş hepsi\`
─────────────────────
...
**Doğru Kullanım:** \`${prefix[0]}çekiliş bulunduğum\`
...
**Kazanan Kullanıcı:** <@${random}> (\`${random}\`)`))
    ...
    let Embed = new Discord.MessageEmbed()
      ...
      .setDescription(`
**Çekilişe katılmak için** :tada: **tepkisine tıklayınız** (İsminde \`${tag}\` tagını bulunduran üyelerin isimleri 2 defa yazılmaktadır!)
...
**Verilecek Ödül:** \`${prize.replace(message.mentions.roles.first(), "")}\`
      `)
 ...
 if (saniye === 0 || saniye < 0) {
  ...
  setTimeout(() => {
    ...
    m.reactions.cache.get("🎉").users.cache.filter((u) => !u.bot && message.author.id !== u.id).map(x => {
      if (x.username.includes(tag) || message.guild.members.cache.get(x.id).roles.cache.get(booster)) {
        arr.push(x.id)
      }
      arr.push(x.id)
    })
...
`)
    ...
    m.edit(Embed)
    channel.send(`**${prize.replace(message.mentions.roles.first(), "")}** için verilen hediyenin galibi. <@${random}>`);
  }, 1000);
...
**Çekilişe katılmak için** :tada: **tepkisine tıklayınız** (İsminde \`${tag}\` tagını bulunduran üyelerin isimleri 2 defa yazılmaktadır!)
**Çekilişi Başlatan:** \`${message.author.username}\`
**Gerçekleşecek Süre:** \`${saat} saat ${dakika+1} dakika kaldı\`
**Verilecek Ödül:** \`${prize.replace(message.mentions.roles.first(), "")}\`
      `)
      .setFooter(client.ayarlar.footer)
      .setTimestamp()
